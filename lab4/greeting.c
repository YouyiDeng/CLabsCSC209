...
int main(int argc, char **argv) {
    ...
    // Your code goes here
    char *greeting_msg = (char*)malloc((strlen(argv[1] + 1 + strlen(name)) * sizeof(char)));
    if (greeting_msg == NULL){
        printf("%s", "Error calling malloc.\n");
        exit(1);
    }
    strcpy(greeting_msg, argv[1]);
    strcat(greeting_msg, " ");
    strcat(greeting_msg, name);
    strncpy(greeting, greeting_msg, sizeof(greeting) - 1);
